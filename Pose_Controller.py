...
class Pose_Controller:
    ...
    def checkPose_UDC(self, img, results):
        ...
        if centerShoulder_y > self.shoulder_line_y + down_threshold:
            UDC = "D"
        elif centerShoulder_y < self.shoulder_line_y - jump_threshold:
            UDC = "U"
        else:
            UDC = "C"
    ...
    def checkClap(self, img, results):
        ...
        cv2.putText(img,CLAP,(10,30), cv2.FONT_HERSHEY_PLAIN, 2, (255,255,0),3)
        
        return img, CLAP
    
    def saveShoulder_y(self, img, results):
        img_height, img_width, _ = img.shape

        leftShoulder_y = int(results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.LEFT_SHOULDER].y * img_height)
        rightShoulder_y = int(results.pose_landmarks.landmark[self.mp_pose.PoseLandmark.RIGHT_SHOULDER].y * img_height)

        self.shoulder_line_y = abs(leftShoulder_y + rightShoulder_y) // 2
